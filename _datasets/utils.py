...
def convert_user_item_pairs_into_sparse_matrix(interactions: pd.DataFrame):
    """
    ...
    if len(interactions) == 0:
        return [], [], csr_matrix(([], ([], [])), shape=(0, 0), dtype=np.float64)
...
def fast_pruning(
    ...
    logger.info(
        """Due to a pruning, the number of unique users and items could changed:
            Users: {} => {}
            Items: {} => {}""".format(users_cnt_old, len(interactions["user_id"].cat.categories), items_cnt_old, len(interactions["item_id"].cat.categories))
    )
    return interactions
