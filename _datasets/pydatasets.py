...
from .utils import get_sparse_matrix_from_dataframe


class BasicRecSysDataset(PyDataset):
    """
...
class PredictDfRecSysDataset(PyDataset):
    """
...
class SparseRecSysDataset(PyDataset):
    """
    input sparse interaction matrix
...
class SparseTransposedRecSysDataset(PyDataset):
    """
...
class __SparseRecSysDatasetWithNegatives(PyDataset):
    """
    input sparse interaction matrix
    output batches of user vectors + slicer with indices of nonzero columns
    """
    ...
    def __init__(
        self, X: scipy.sparse.csr_matrix, device, batch_size: int = 32, shuffle=False, workers=1, use_multiprocessing=False, max_queue_size=10, max_output=None
    ):
        super().__init__(workers=workers, use_multiprocessing=use_multiprocessing, max_queue_size=max_queue_size)
        self.X, self.batch_size, self.shuffle = X, batch_size, shuffle
        self.indices = np.arange(X.shape[0])
    ...
    def __getitem__(self, n):
        ...
        if self.max_output - len(item_slicer) > 0:
            item_slicer_with_negatives = np.hstack([item_slicer, np.random.choice(self.items_indices[mask], self.max_output - len(item_slicer))])
        else:
...
class SparseRecSysDatasetWithNegatives(PyDataset):
    """
    input sparse interaction matrix
    output batches of user vectors + slicer with indices of nonzero columns
    """
