...
if __name__ == "__main__":
    ...
    if args.decay_strategy == "None":
        for val in vals:
            ...
            model = CompressedSparseKerasELSA(
                X.shape[1], dim, df.item_id.cat.categories, device=DEVICE, n_vals=val, embs=embs_, level_dim=level_dim, n_levels=n_levels, add_4k=ADD4K
            )
            model.to(DEVICE)
        ...
        start = time()
        model = CompressedSparseKerasELSA(
            X.shape[1], dim, df.item_id.cat.categories, device=DEVICE, n_vals=target_vals, embs=embs_, level_dim=level_dim, n_levels=n_levels, add_4k=ADD4K
        )
        model.to(DEVICE)
        data_loader = SparseRecSysDatasetWithNegatives(X, device=DEVICE, batch_size=args.batch_size, shuffle=args.shuffle, max_output=args.max_output)
        model.compile(optimizer=NadamS(learning_rate=lr, weight_decay=args.weight_decay), loss=NMSE, metrics=[keras.metrics.CosineSimilarity()])
