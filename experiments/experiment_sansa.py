...
from recommenders.sansa_model import SansaRecommender
...
parser.add_argument(
    ...
    default=3,
    type=float,
    help="SANSA: number of refinement passes through entire matrix. later scans are more precise and expensive. 0-3 recommended",
)
parser.add_argument(
    "--umr_finetune_steps",
    default=10,
    type=float,
    help='SANSA: number of refinements improving "several worst columns". relatively inexpensive. typically 0-20',
)
...
parser.add_argument(
    "--ldlt_method", default="icf", type=str, help="SANSA: cholmod or icf (cholmod is exact factorization + sparsification, icf is incomplete factorization)"
)
...
if __name__ == "__main__":
    ...
    model = SansaRecommender(
        item_idx=dataset.full_train_interactions.item_id.cat.categories,
        lambda_=args.lam,
        target_density=args.target_density,
        umr_scans=args.umr_scans,
        umr_finetune_steps=args.umr_finetune_steps,
        umr_loss_threshold=args.umr_loss_threshold,
